...
<head>
<title>Product List | Apni Dukan Online | Online Shopping in Lucknow | Best Online Shopping Website in Lucknow | Best Home Products for Online Sale	| Best Shopping website in Lucknow | Dukan Online</title>
<script type="text/javascript">
function callPage(id,valu){
	location.href="search?hint="+id+"&order="+valu;
}
</script>
</head>
<body>
...
<jsp:include page="header.jsp" />
 <div class="main">
    <div class="content">
		...
		String subcateg= (String)request.getParameter("hint");
		Query q1 = pm.newQuery(Product.class);
    	//q1.setFilter("productName==subcatParam");
    //	q1.declareParameters("String subcatParam");
    	q1.setOrdering("productName desc");
    	...
    	try {
    		listProduct = (List<Product>) q1.execute();
    		//System.out.println(listProduct.size());
    	%>
    	<div class="content_top">
    		<div class="heading">
    		<h4>Search results for: <%=subcateg %></h4>
    		</div>
    		<div class="sort">
    		<p>Sort by:
				...
				<%
					int counter=0;
						for(Product product:listProduct){
							//System.out.println(product.getProductName()+"contains("+subcateg+")="+product.getProductName().contains(subcateg));
							if (product.getProductName().toLowerCase()
									.contains(subcateg.toLowerCase())
									|| product.getCategory().toLowerCase()
											.contains(subcateg.toLowerCase())
									|| product.getInnerSubCategory().toLowerCase()
											.contains(subcateg.toLowerCase())
									|| product.getCategory().toLowerCase()
											.contains(subcateg.toLowerCase())
